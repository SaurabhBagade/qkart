...
const Checkout = () => {
    ...
    const addAddress = async (token, newAddress) => {
        ...
        try {
            ...
            let data = await axios.post(`${config.endpoint}/user/addresses`,
            ...
            setAddresses({ all: resp, selected: "" })
            return resp
        } catch (e) {
            if (e.response) {
                enqueueSnackbar(e.response.data.message, { variant: "error" });
    ...
    const deleteAddress = async (token, addressId) => {
        ...
        try {
            ...
            let data = await axios.delete(`${config.endpoint}/user/addresses/${addressId}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
    ...
    const validateRequest = (items, addresses) => {
        ...
        if (items.length === 0) {
            enqueueSnackbar("Currently you do not have any item selected in cart. Please select a item !", {
                variant: "warning",
                autoHideDuration: 3000
            })
        } else if (balance < getTotalCartValue(items)) {
    ...
    const performCheckout = async (token, items, addresses) => {
        ...
        try {
            ...
            if (e.response) {
                ...
                enqueueSnackbar("Could not place order. Check that backend is running, reachable and returns a valid JSON.", { variant: "error" })
            }
            return false;
        }
                        ...
                        <Box>
                            ...
                            {addresses.all.length
                                ? addresses.all.map((address) => {
                                    return (<Box onClick={() => setAddresses({ ...addresses, selected: address._id })} className={addresses.selected === address._id ? "address-item selected" : "address-item not-selected"} key={address._id} >
                                        <p>{address.address}</p>
                                        <Button variant="text" onClick={async () => await deleteAddress(token, address._id)} startIcon={<DeleteIcon />}>
                                            Delete
                                        </Button>
                                    </Box>)
                                })
